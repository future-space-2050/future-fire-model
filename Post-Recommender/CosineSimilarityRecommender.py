...
class CosineSimilarityRecommender:
    def __init__(self, posts_df, user_data=None):
        ...
        self.posts_df = posts_df.reset_index(drop=True).copy()
        self.posts_df['post_id'] = posts_df.index  # Store original IDs
    ...
    def _get_cached_embeddings(self, emb_file=EMBEDDING_FILE_PATH):
        ...
        if os.path.exists(emb_file):
            ...
            if len(cached) == len(self.posts_df):
...
if __name__ == "__main__":
    ...
    users['interacted_posts'] = ""
    users['interacted_posts'] = users['interacted_posts'].apply(
        lambda x: x.strip("[]").replace("'", "").split(", ") if pd.notna(x) else []
    )
    single_user = {
        "Profession": "Musician",
    ...
    recommender = CosineSimilarityRecommender(posts, single_user)
    print("Recommendations:", recommender.recommend(11, use_faiss=False))